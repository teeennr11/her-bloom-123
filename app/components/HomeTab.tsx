...
export default function HomeTab({ userName, day, phase, cycles, sorted, avg, pred, edit, onSave, onEdit, onDelete, onCancelEdit }: Props) {
  // props สำหรับ svg icon ที่ใช้ซ้ำใน InfoCard ทุกอัน
  const ip = { className: "w-3.5 h-3.5 text-[#FF2878]", viewBox: "0 0 20 20", fill: "none", stroke: "currentColor", strokeWidth: "1.7" };
  const latest = sorted[0]; // record ล่าสุด (sorted เรียงจากใหม่ไปเก่า)
    ...
    <div className="fu space-y-6 pt-[10px] md:pt-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 items-stretch mb-6">
        ...
        <div className="flex flex-col gap-2.5">
          {/* Card วงแหวนแสดง phase ปัจจุบัน */}
          <div className="flex-1 rounded-[28px] px-5 pt-[30px] pb-[22px] text-center relative overflow-hidden bg-white border border-[#F0CCD8] shadow-[0_6px_28px_rgba(255,40,120,0.07)]">
            {/* Radial gradient พื้นหลังตกแต่ง */}
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {/* การ์ดวันที่คาดว่าประจำเดือนจะมาครั้งถัดไป */}
            <InfoCard
              ...
              sub={pred ? `in ${Math.max(0, daysUntil(pred.nextPeriod))} days` : "Log to predict"} /> {/* daysUntil คำนวณจำนวนวันที่เหลือ */}
            {/* การ์ดช่วง fertile window */}
            <InfoCard
            ...
            {/* การ์ด Pregnancy chance ของ phase ปัจจุบัน */}
            <div className="rounded-[18px] p-4 bg-white border border-[#F0CCD8] shadow-[0_4px_20px_rgba(255,40,120,0.08)]">
              ...
              {/* phase.pregnancy = ข้อความระดับโอกาสตั้งครรภ์ เช่น "Low", "High" */}
            ...
            {/* การ์ดวันที่ประจำเดือนมาครั้งล่าสุด */}
            <InfoCard
              ...
              sub={latest ? `${avg}-day avg` : "No records yet"} /> {/* avg = ความยาวรอบเฉลี่ย */}
          </div>
        </div>
      ...
      {/* ===== History section ===== */}
      <div>
        {/* หัวข้อ History พร้อมแสดงจำนวน record ทั้งหมด */}
        <div className="flex items-baseline justify-between mb-3.5">
          <h3 className="font-serif text-[20px] md:text-[22px] font-bold text-[#1A0A10]">History</h3>
          <span className="font-sans text-[10px] text-[#C8A0B0]">{cycles.length} record{cycles.length !== 1 ? "s" : ""}</span> {/* plural "records" เมื่อมากกว่า 1 */}
        </div>

        {/* Empty state เมื่อยังไม่มี record */}
        {sorted.length === 0 ? (
          ...
          <div>
            {sorted.map(c => (
              <HistoryRow
                ...
                onEdit={c => { onEdit(c); window.scrollTo({ top: 0, behavior: "smooth" }); }} // เลื่อนขึ้นบนหน้าเพื่อแก้ไขในฟอร์ม
                onDelete={onDelete}
              />
            ))}
          </div>
