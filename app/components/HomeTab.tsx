//  app/components/HomeTab.tsx  —  หน้าหลัก (แท็บ Home)
"use client";
...
// Props ทั้งหมดที่รับจาก page.tsx
type Props = {
  userName: string;                           // ชื่อผู้ใช้
  day: number;                                // วันที่ในรอบปัจจุบัน
  phase: Phase;                               // phase ปัจจุบัน
  cycles: Cycle[];                            // ข้อมูลดิบทั้งหมด (ไม่เรียง)
  sorted: Cycle[];                            // ข้อมูลเรียงจากใหม่→เก่า
  avg: number;                                // ความยาวรอบเฉลี่ย
  pred: Pred | null;                          // ผลทำนาย (null ถ้าข้อมูลน้อยไป)
  edit: Cycle | null;                         // รอบที่กำลัง edit
  onSave: (f: Form, id?: string) => void;    // บันทึก
  onEdit: (c: Cycle) => void;                // กด edit
  onDelete: (id: string) => void;            // ลบ
  onCancelEdit: () => void;                  // ยกเลิก edit
};

export default function HomeTab({
  userName, day, phase, cycles, sorted, avg, pred,
  edit, onSave, onEdit, onDelete, onCancelEdit
}: Props) {

  // ip = props สำหรับ SVG ไอคอนใน InfoCard (ใช้ซ้ำหลายครั้ง)
  const ip = {
    className: "w-3.5 h-3.5 text-[#FF2878]",
    viewBox: "0 0 20 20", fill: "none",
    stroke: "currentColor", strokeWidth: "1.7",
  };

  const latest = sorted[0]; // รอบล่าสุด (index 0 เพราะเรียงจากใหม่→เก่า)

  return (
    // pt เพิ่มบน mobile เพื่อเว้นจาก mobile strip
    <div className="fu space-y-6 pt-[10px] md:pt-0">

      {/* ── Header: ทักทาย + badge phase ── */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4 mb-4">
        <div>
          ...
          <p className="font-sans text-[10px] font-normal mt-1.5 tracking-[0.14em] text-[#C8A0B0] uppercase">
            {/* แสดงวันที่วันนี้แบบเต็ม */}
            {new Date().toLocaleDateString("en-GB", {
              weekday: "long", day: "numeric", month: "long", year: "numeric",
            })}
          </p>
        </div>
        {/* Badge แสดง Phase + Day */}
        ...
        <div className="flex flex-col gap-2.5">
            ...
            <div className="flex justify-center gap-3 md:gap-3.5 mt-[18px] flex-wrap relative z-10">
              {[
                ...
                <div key={x.l} className="font-sans flex items-center gap-1 text-[9px] font-normal text-[#C8A0B0]">
                  <div className={`w-[7px] h-[7px] rounded-full ${x.c}`} />
                  {x.l}
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            ...
            <InfoCard
              ...
              label="Next Period"
              value={pred ? fmtS(pred.nextPeriod) : "—"}
            ...
            <InfoCard
              ...
              value={pred ? `${fmtS(pred.fertileStart)}–${fmtS(pred.fertileEnd)}` : "—"}
              sub="ovulation ± 5 days"
            />

            {/* Pregnancy Chance (custom card ไม่ใช้ InfoCard) */}
            <div className="rounded-[18px] p-4 bg-white border border-[#F0CCD8] shadow-[0_4px_20px_rgba(255,40,120,0.08)]">
              ...
              <span className="font-sans inline-block px-[11px] py-[3px] rounded-full text-[10px] font-semibold bg-[#FFF0F6] text-[#FF2878] border border-[#F0CCD8]">
                ● {phase.pregnancy} {/* โอกาสตั้งครรภ์ของ phase ปัจจุบัน */}
              </span>
            ...
            <InfoCard
              ...
              label="Last Period"
              value={latest ? fmtS(new Date(latest.startDate)) : "—"}
              sub={latest ? `${avg}-day avg` : "No records yet"}
            />
          </div>
        </div>

        {/* คอลัมน์ขวา: CycleForm */}
      ...
      <div>
        <div className="flex items-baseline justify-between mb-3.5">
          ...
          <span className="font-sans text-[10px] text-[#C8A0B0]">
            {cycles.length} record{cycles.length !== 1 ? "s" : ""}
            {/* "1 record" หรือ "3 records" */}
          </span>
        </div>

        {/* ถ้าไม่มีข้อมูลแสดง placeholder */}
        {sorted.length === 0 ? (
          ...
          <div>
            {sorted.map(c => (
              <HistoryRow
                ...
                phase={phase}
                onEdit={c => {
                  onEdit(c);                                      // เซ็ต edit state
                  window.scrollTo({ top: 0, behavior: "smooth" }); // scroll ขึ้นบนเพื่อโชว์ฟอร์ม
                }}
                onDelete={onDelete}
              />
            ))}
